...
public class ScanActivity extends Activity implements Callback,OnClickListener{
			...
			case R.id.ibLignt:
				try {
					...
					if(parameters.getFlashMode().equals(Parameters.FLASH_MODE_TORCH)){
						...
						ibLignt.setBackgroundResource(R.mipmap.ic_light_close);

					}else{
						parameters.setFlashMode(Parameters.FLASH_MODE_TORCH);
						ibLignt.setBackgroundResource(R.mipmap.ic_light_open);
					}
					CameraManager.get().camera.setParameters(parameters);
					CameraManager.get().camera.autoFocus(CameraManager.get().autoFocusCallback);
				} catch (Exception e) {
					ToastUtils.show(context, "设备故障");
				}
				break;
			case R.id.ibPhoto:
				//打开手机中的相册
	...
	public void handleDecode(Result result, Bitmap barcode) {
		...
		if(!StringUtils.isEmpty(scan_res)){
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode == RESULT_OK){
			switch(requestCode){
				case REQUEST_CODE:
//					Log.d(context, "返回的地址：" + data.getData().toString());
					//获取选中图片的路径
					...
					if (cursor != null && cursor.moveToFirst()) {
						...
						ToastUtils.show(context,"无法访问图片地址！");
					}
					break;
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				...
				case PARSE_BARCODE_FAIL:
					ToastUtils.show(context, (String) msg.obj);
					break;
	...
	private void onResultHandler(String resultString, Bitmap bitmap){
		if(TextUtils.isEmpty(resultString)){
			ToastUtils.show(context,"Scan failed!");
			return;
		}
